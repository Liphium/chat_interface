...
String encryptSymmetric(String data, SecureKey key, [Sodium? sd]) {
  ...
  return base64Encode(nonce +
      sodium.crypto.secretBox
          .easy(key: key, nonce: nonce, message: plainTextBytes));
}
...
Uint8List encryptSymmetricBytes(Uint8List data, SecureKey key, [Sodium? sd]) {
  ...
  final nonce = sodium.randombytes.buf(sodium.crypto.secretBox.nonceBytes);
  final finalList =
      Uint8List(nonce.length + data.length + sodium.crypto.secretBox.macBytes);
  finalList.setAll(0, nonce);
  finalList.setAll(nonce.length,
...
String decryptSymmetric(String data, SecureKey key, [Sodium? sd]) {
  ...
  return String.fromCharCodes(sodium.crypto.secretBox
      .openEasy(key: key, nonce: nonce, cipherText: encrypted));
}
