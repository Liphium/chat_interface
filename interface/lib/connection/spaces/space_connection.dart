...
Future<bool> createSpaceConnection(String domain, String token) async {
  return await spaceConnector
      .connect("ws://$domain/gateway", token, restart: false, onDone: ((error) {
    if (error) {
...
void setupSpaceListeners() {
  ...
  spaceConnector.listen(
      "room_data", (event) => handleRoomData(event)); // Sent on change
  spaceConnector.listen(
      "room_info", (event) => handleRoomData(event)); // Sent on join
  spaceConnector.listen("member_update",
      (event) => handleMemberUpdate(event)); // Sent on member update
...
void handleRoomData(Event event) {
  ...
  controller.start.value =
      DateTime.fromMillisecondsSinceEpoch(event.data["start"]);
