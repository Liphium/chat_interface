...
class MemberContainer {
  ...
  String encrypted(SecureKey key) =>
      encryptSymmetric(jsonEncode(<String, dynamic>{"id": id}), key);
}
...
class ConversationContainer {
  ...
  ConversationContainer.fromJson(Map<String, dynamic> json)
      : name = json["name"];
...
Future<bool> openDirectMessage(Friend friend) async {
  final conversation =
      Get.find<ConversationController>().conversations.values.firstWhere(
            (element) =>
...
Future<bool> _openConversation(List<Friend> friends, String name) async {
  if (Get.find<ConversationController>().conversations.length >=
      specialConstants["max_conversation_amount"]) {
    showErrorPopup(
        "conversations.error".tr,
        "conversations.amount".trParams({
  ...
  final ownMemberContainer =
      MemberContainer(Get.find<StatusController>().id.value)
  ...
  sendLog(
      "${name.length} | $name | ${specialConstants["max_conversation_name_length"]}");

  if (name.length > specialConstants["max_conversation_name_length"]) {
    showErrorPopup(
        "conversations.error".tr,
        "conversations.name.length".trParams({
  ...
  if (friends.length > specialConstants["max_conversation_members"]) {
    showErrorPopup(
        "conversations.error".tr,
        "conversations.members.size".trParams(
  ...
  for (var friend in friends) {
    ...
    final signature = signMessage(
        signatureKeyPair.secretKey, "${body["conversation"]}${friend.id}");
  ...
  await conversationController.addCreated(conversation, members,
      admin: Member(
          conversation.token.id, statusController.id.value, MemberRole.admin));
  subscribeToConversation(statusController.statusJson(),
      statusController.generateFriendId(), conversation.token,
      deletions: false);
