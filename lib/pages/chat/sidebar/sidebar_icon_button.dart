...
class _SidebarButtonState extends State<SidebarIconButton> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    return Animate(
      ...
      effects: [ScaleEffect(begin: const Offset(0.75, 0.75), end: const Offset(1.0, 1.0), curve: Curves.easeOut, duration: 120.ms)],
      child: Obx(() => Material(
            borderRadius: const BorderRadius.only(
              bottomLeft: Radius.circular(defaultSpacing),
              topRight: Radius.circular(defaultSpacing),
            ),
            color: widget.selected.value == widget.index ? Theme.of(context).colorScheme.primary : Theme.of(context).colorScheme.onInverseSurface,
            child: InkWell(
              borderRadius: const BorderRadius.only(
                bottomLeft: Radius.circular(defaultSpacing),
                topRight: Radius.circular(defaultSpacing),
