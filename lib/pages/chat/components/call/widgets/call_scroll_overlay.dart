...
class CallPositionOverlay extends StatefulWidget {
  ...
  const CallPositionOverlay({super.key, required this.position, required this.alignment, required this.show});
...
class _CallPositionOverlayState extends State<CallPositionOverlay> {
  ...
  Widget build(BuildContext context) {
    ...
    if(horizontal) {
      alignment = widget.alignment == CallOverlayAlignment.start ? Alignment.topCenter : Alignment.bottomCenter;
    } else {
      alignment = widget.alignment == CallOverlayAlignment.start ? Alignment.centerLeft : Alignment.centerRight;
    }
  ...
  Widget horizontalButton(SettingController controller, CallOverlayPosition position) {

    return IconButton(
      onPressed: () => controller.settings["call_app.expansionPosition"]!.setValue(position == CallOverlayPosition.right ? 3 : 1),
      icon: Icon(position == CallOverlayPosition.right ? Icons.arrow_back : Icons.arrow_forward, size: 25,)
    );
  }

  Widget verticalButton(SettingController controller, CallOverlayPosition position) {

    return IconButton(
      onPressed: () => controller.settings["call_app.expansionPosition"]!.setValue(position == CallOverlayPosition.top ? 2 : 0),
      icon: Icon(position == CallOverlayPosition.top ? Icons.arrow_downward : Icons.arrow_upward, size: 25,)
    );
  }
